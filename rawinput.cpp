...
void CRawInput::DetectRemoval(LPARAM lParam, WPARAM wParam)
{
	WCHAR ridDeviceName[256] = { 0 };
	UINT dwSize = 256;
	UINT dwResult = GetRawInputDeviceInfoW((void*)lParam, RIDI_DEVICENAME, &ridDeviceName, &dwSize);
	...
	else
	{
		...
		UINT dwResult = GetRawInputDeviceInfoW((void*)lParam, RIDI_DEVICEINFO, &info, &infosize);
		int ret = this->Detection((HANDLE)lParam, info.dwType);
		//if (ret >= 0)
		//{
		//	std::shared_ptr<RInputController> p = std::make_shared< RInputController>();
		//	p->Init((LPVOID)iman.Raw.device[ret]);
		//	iman.Pads[ret] = p;
		//	iman.SetActiveController(config.inputLast);
		//}
	}
}
